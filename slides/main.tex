...
\begin{frame}
  ...
  \begin{columns}[onlytextwidth]
    ...
    \begin{column}{0.45\linewidth}
      ...
      \onslide<2> {\begin{block}{Esp. Ing. Daniela Armijo} 
        \begin{itemize}
          \item Ing. Química
          \item Especialista en Docencia Univ. de UNCuyo
          \item Docente en: Análisis Matematico I, Algebra y Geometria Analitica, Fisica I, y Análisis Matematico II 
          \item Estudiante en la Tecnicatura en Programación de FRSR - UTN. 
        \end{itemize}}
      \end{block}
    ...
    \begin{column}{0.45\linewidth}
      \onslide<3> {\begin{block}{Ing. Paola B. Torres}
         \begin{itemize}
           \item Ing. Química 
           \item Estudiante de Doctorado en FRSR - UTN 
           \item Grupo Vinculado Bionanotecnología y sistemas complejos FRSR UTN-INFAP 
           \item Ayudante en Sistemas Dinamicos I
         \end{itemize}
       \end{block}}
     \end{column}
  ...
  \end{figure}
  % Soft matter Research Group at Charles University, Prague, Czech Republic. 
  ...
  % \begin{figure}
  %   \includegraphics[width=0.2\textwidth]{images/logo_conicet.png}
  % \end{figure}
...
\begin{frame}{Creación de un repositorio Git} 
...
\onslide<1>{\begin{block}{Conectar repositorio a la nube de GitHub}
  \small 
...
\subsection {Confirmar cambios en el repositorio}
  ...
  \begin{frame} {Confirmar cambios en el repositorio}
    ...
    \onslide<1->{\begin{block}{Primero: Agregar archivos }
      {\texttt{\$ git add [archivo] }}
    \end{block}}

    \onslide<2->{   \begin{block}{Segundo: git commit}
      {\texttt{\$ git commit -m `insertar mensaje simple y preciso' }}
    \end{block}}

    \onslide<3->{\begin{block}{Tercero: git push}
      {\texttt{\$ git push}}
    \end{block}}
...
\begin{frame} {Gestión de archivos}
  ...
  \onslide<1->{\begin{block}{Agregar archivos }
    {\texttt{\$ git add <nombre\_archivo> }}
  \end{block}}

  \onslide<2-> {  \begin{block}{Mover archivos}
    \texttt{\$ git mv <nombre\_archivo>}
  \end{block}}

  \onslide<3->{  \begin{block}{Elimnar archivos}
...
\subsection {Historial de cambios y estado del repositorio}
  ...
  \begin{frame} {\LARGE Historial de cambios y estado del repositorio}
    ...
    \onslide<1>{\begin{block}{Estado de los archivos}
      {\texttt{\$ git status}} 
    \end{block}}

    \onslide<2>{\begin{block}{Historial}
      {\texttt{\$ git log}} \\
...
\begin{frame}{Identificar commits}
  ...
  \onslide<2> {\begin{alertblock}{¿Qué es un Hash?}
...
\begin{frame}
  ...
  \onslide<2>{ \begin{block}{¿Para que nos sirve el Hash?}
      \texttt{\$ git diff <hash\_a> <hash\_b>} \\
      \texttt{\$ git diff $-$$-$name$-$only <hash\_a> <hash\_b>} \\
      \texttt{\$ git checkout <hash\_commit> } \\ 
      \texttt{\$ git reset $-$$-$ hard <hash>}
